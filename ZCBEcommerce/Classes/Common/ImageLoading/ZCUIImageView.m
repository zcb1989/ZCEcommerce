...
void ZCImageCacheClearWithCompletion(dispatch_block_t completion){
    
    [[SDImageCache sharedImageCache] clearDiskOnCompletion:completion];
    [[SDImageCache sharedImageCache] clearMemory];
}

void ZCImageCachePreloadImages(NSArray *urls){
    
    [urls enumerateObjectsUsingBlock:^(id obj,NSUInteger idx,BOOL *stop){
        if (![[SDWebImageManager sharedManager] cachedImageExistsForURL:obj]) {
            [[SDWebImageManager sharedManager] downloadImageWithURL:obj options:SDWebImageRetryFailed|SDWebImageContinueInBackground progress:NULL completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                ZCLogInfo(@"Preload %@ OK",imageURL);
            }];
        }
    }];
}

id <ZCImageLoadOperation> ZCImageLoadImage(NSURL *url,void(^completion)(UIImage *image)){
...
void ZCImageCacheClearMemory(void){
    [[SDImageCache sharedImageCache] clearMemory];
}
...
+ (instancetype)captchaView{
    ZCUIImageView *view = [[self alloc] init];
...
- (void)setTouchEndBlock:(void (^)(ZCUIImageView *))touchEndBlock{
...
- (void)setUp{
    [super setUp];
    @weakify(self);
    [self setTouchEndBlock:^(ZCUIImageView *imageView) {
        @strongify(self);
